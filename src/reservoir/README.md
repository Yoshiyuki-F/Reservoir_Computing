...
4. Model (Engine) - [Batch, Time, Hidden] -> [Batch, Time, Hidden] (Reservoir) OR [Batch, Hidden] (FNN)
Role: Stateful dynamics or Deep Non-linear mapping.
Examples: Classical Reservoir, Quantum Reservoir, FNN (Student)
Adapter: Flatten is used here if Model requires 2D input (e.g. [Batch, Time*Hidden] -> FNN).
5. Aggregation - [Batch, Time, Hidden] -> [Batch, Feature]
Role: Temporal reduction to fixed feature vector.
Examples: Last state, Mean, Concat
6. Readout - [Batch, Feature] -> [Batch, Output]
