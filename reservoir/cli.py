"""
JAXを使ったReservoir ComputingのCommand Line Interface。

Usage:
    python -m reservoir.cli --config configs/sine_wave_demo_config.json
    python -m reservoir.cli --config configs/lorenz_demo_config.json
    python -m reservoir.cli --sine --config configs/sine_wave_demo_config.json
    python -m reservoir.cli --lorenz --config configs/lorenz_demo_config.json
"""
...
# デフォルト設定ファイルパス
DEFAULT_SINE_CONFIG_PATH = 'configs/sine_wave_demo_config.json'
DEFAULT_LORENZ_CONFIG_PATH = 'configs/lorenz_demo_config.json'
  ...
  python -m reservoir.cli --all
    ...
    try:
        ...
        if args.config:
            ...
            config_files_to_run = [
                DEFAULT_SINE_CONFIG_PATH,
                DEFAULT_LORENZ_CONFIG_PATH
            ]
