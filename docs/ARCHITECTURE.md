...
* **5.5. Feature Extraction (All Paths)**
    * **Input:** `[N, Raw_Features]` (Train/Val/Test splits)
    * **Action:** 学習済みのモデル（Reservoir または FNN）を用いて、全データに対する最終的な特徴量ベクトルを抽出します。
    * **Output:** `[N', Hidden]` (Train_Z, Val_Z, Test_Z)
